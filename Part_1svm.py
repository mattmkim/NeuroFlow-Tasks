...
# create a 3D plot that plots sleep, mood, and stress
fig = pyplot.figure()
ax = Axes3D(fig)
...
# concatenate columns to create one dataframe that has all metrics
...
# create SVM, test data will be a portion of the data from the original dataset
combine = X.values
X_train = combine
